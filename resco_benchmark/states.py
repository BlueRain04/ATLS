...
def drq(signals): #iterate over each signal, each lane in the signal, it the lane is green => add 1, then take lane info as obsrvation, also calculate vehicles speed as a signal obsrvation
    observations = dict()
...
def drq_norm(signals):#same as the previous function but data are normalized
    observations = dict()
    for signal_id in signals:
        signal = signals[signal_id]
        obs = []
...
def mplight(signals):#iterate over signals, for each direction (North, South, East, West), for each incoming lane, add the queue_length, then iterate over outgoing lanes, if the lane is neighboring traffic signal then subtract 
    observations = dict()
    for signal_id in signals:
        signal = signals[signal_id]
        obs = [signal.phase]
...
def mplight_full(signals):#same as previous but has more info and they are normalized
    observations = dict()
    for signal_id in signals:
        signal = signals[signal_id]
        obs = [signal.phase]
...
def wave(signals):#iterate over each signal, for each incoming lane, add the queue length and approaching vehicles as the wave, then add it to state list, then add it to observation to signal
    observations = dict()
    for signal_id in signals:
        signal = signals[signal_id]
        state = []
...
def fma2c(signals):
    ...
    region_fringes = dict()#for each signal, each neighbor, when the lane is connecting two different regions then add it to a dict
    ...
    lane_wave = dict()#calculate the wave for each signal and lane
    for signal_id in signals:
        signal = signals[signal_id]
        for lane in signal.lanes:
            lane_wave[lane] = signal.full_observation[lane]['queue'] + signal.full_observation[lane]['approach']

    manager_obs = dict()#first observation is the manager which is considering the wave in the lanes going outside the region 
    ...
    management_neighborhood = dict()#we will add the neighborhood observation to the manager, that the neighborhood observation and multi with alpha (how much to consider neighbors) 
    ...
    signal_wave = dict()#same as the wave one, but here add each wave for the signal with clipping and norm
    for signal_id in signals:
        signal = signals[signal_id]
        waves = []
        for lane in signal.lanes:
    ...
    observations = dict()#for each signal, for each neighbor, when the two signals are from the same region, then add it to the signal observation with alpha (how much to consider neighbors)
    for signal_id in signals:
        signal = signals[signal_id]
        waves = [signal_wave[signal_id]]
        for key in signal.downstream:
        ...
        waits = []#add the wait for the signal, with norm and clipping
        for lane in signal.lanes:
            max_wait = signal.full_observation[lane]['max_wait']
            waits.append(max_wait)
...
def fma2c_full(signals):
    ...
    management_neighborhood = dict()#later
    for manager in manager_obs:
        neighborhood = [manager_obs[manager]]
        for neighbor in management_neighbors[manager]:
            neighborhood.append(fma2c_config['alpha'] * manager_obs[neighbor])
